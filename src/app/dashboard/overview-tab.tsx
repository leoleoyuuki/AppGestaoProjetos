...
export default function OverviewTab() {
  ...
  const metrics = useMemo(() => {
    ...
    const monthlyRevenues = userRevenues.filter(r =>
      isWithinInterval(parseISO(r.transactionDate), { start: monthStart, end: monthEnd })
    );
    const monthlyCosts = costs.filter(c =>
      isWithinInterval(parseISO(c.transactionDate), { start: monthStart, end: monthEnd })
    );
    ...
    const receivableItems = userRevenues.filter(r =>
      (!r.receivedAmount || r.receivedAmount === 0) && isWithinInterval(parseISO(r.transactionDate), { start: today, end: thirtyDaysFromNow })
    );
    const totalReceivable = receivableItems.reduce((acc, r) => acc + r.plannedAmount, 0);

    const payableItems = costs.filter(c =>
      (!c.actualAmount || c.actualAmount === 0) && isWithinInterval(parseISO(c.transactionDate), { start: today, end: thirtyDaysFromNow })
    );
